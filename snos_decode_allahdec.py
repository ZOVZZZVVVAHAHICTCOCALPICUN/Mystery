...
def menu():  #Декоднул @allahdec.
    print(colored(banner, 'magenta'))
    choice = input('Введите номер: ')
    return choice  #Декоднул @allahdec.


def send_email(receiver, sender_email, sender_password, subject, body):  #Декоднул @allahdec.
...
def main():  #Декоднул @allahdec.
    ...
    if choice == '1':  #Декоднул @allahdec.
        print(colored(banner1, 'magenta'))
        comp_choice = input('Выберите способ: ')

        if comp_choice in ('1', '2', '3'):  #Декоднул @allahdec.
            ...
            for sender_email, sender_password in senders.items():  #Декоднул @allahdec.
                for receiver in receivers:  #Декоднул @allahdec.
                    comp_text = comp_texts[comp_choice]
                    comp_body = comp_text.format(username=username.strip(), id=id.strip(),
                                                  chat_link=chat_link.strip(), violation_link=violation_link.strip())
        ...
        elif comp_choice == '4':  #Декоднул @allahdec.
            username = input('@username: ')
            id = input('Telegram id: ')

            comp_texts = {
                '4': 'Здравствуйте, уважаемая поддержка. Я случайно перешел по фишинговой ссылке и утерял доступ к своему аккаунту. Его юзернейм - {username}, его айди - {id}. Пожалуйста удалите аккаунт или обнулите сессии'
            }
            for sender_email, sender_password in senders.items():  #Декоднул @allahdec.
                for receiver in receivers:  #Декоднул @allahdec.
                    comp_text = comp_texts[comp_choice]
                    ...
                    time.sleep(5)
            return None  #Декоднул @allahdec.
        elif comp_choice in ('5', '6'):  #Декоднул @allahdec.
            username = input('username: ')
            id = input('Telegram Id: ')
            comp_texts = {
                '5': 'Добрый день поддержка Telegram!Аккаунт {username} , {id} использует виртуальный номер купленный на сайте по активации номеров. Отношения к номеру он не имеет, номер никак к нему не относиться.Прошу разберитесь с этим. Заранее спасибо!',
                '6': 'Добрый день поддержка Telegram! Аккаунт {username} {id} приобрёл премиум в вашем мессенджере чтобы рассылать спам-сообщения и обходить ограничения Telegram.Прошу проверить данную жалобу и принять меры!'
            }
            for sender_email, sender_password in senders.items():  #Декоднул @allahdec.
                for receiver in receivers:  #Декоднул @allahdec.
                    comp_text = comp_texts[comp_choice]
        ...
        print(colored(banner2, 'magenta'))
        ch_choice = input('Выберите способ: ')
        if ch_choice in ('1', '2', '3', '4'):  #Декоднул @allahdec.
            channel_link = input('Ссылка на канал: ')
            channel_violation = input('Ссылка на нарушение: ')
            comp_texts = {
                '1': 'Здравствуйте, уважаемая поддержка телеграм. На вашей платформе я нашел канал, который распространяет личные данные невинных людей. Ссылка на канал - {channel_link}, сслыки на нарушения - {channel_violation}. Пожалуйста заблокируйте данный канал.',
                '2': 'Здравствуйте, уважаемая поддержка телеграма. На вашей платформе я нашел канал который распространяет жестокое обращение с животными. Ссылка на канал - {channel_link}, сслыки на нарушения - {channel_violation}. Пожалуйста заблокируйте данный канал.',
                '3': 'Здравствуйте, уважаемая поддержка телеграма. На вашей платформе я нашел канал который распространяет порнографию с участием несовершеннолетних. Ссылка на канал - {channel_link}, сслыки на нарушения - {channel_violation}. Пожалуйста заблокируйте данный канал.',
                '4': 'Здравствуйте,уважаемый модератор телеграмм,хочу пожаловаться вам на канал,который продает услуги доксинга, сваттинга. Ссылка на телеграмм канал:{channel_link} Ссылка на нарушение:{channel_violation} Просьба заблокировать данный канал.'
            }
            for sender_email, sender_password in senders.items():  #Декоднул @allahdec.
                for receiver in receivers:  #Декоднул @allahdec.
        ...
        print(colored(banner3, 'magenta'))
        bot_ch = input('Выберите: ')
        if bot_ch == '1':  #Декоднул @allahdec.
            bot_user = input('@username: ')
            comp_texts = {
                '1': 'Здравствуйте, уважаемая поддержка телеграм. На вашей платформе я нашел бота, который осуществляет поиск по личным данным ваших пользователей. Ссылка на бота - {bot_user}. Пожалуйста разберитесь и заблокируйте данного бота.'
            }
            for sender_email, sender_password in senders.items():  #Декоднул @allahdec.
                for receiver in receivers:  #Декоднул @allahdec.
